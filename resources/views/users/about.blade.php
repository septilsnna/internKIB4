@extends('layout/main')

@section('title', 'Tentang Kami | Kampus Indonesia')

@section('container')
<div class="container my-4 py-5" style="font-size: 18px; font-family: 'Quicksand', sans-serif; color: #163254;">
    <div class="container-fluid">
        <div class="row align-items-center">
            <div class="col-md-7">
                <h2 style="font-size: 42px;">Bingung Cari Informasi</h2>
                <h2 style="font-size: 36px;">Seputar Kuliah ?</h2>
                <h2 style="font-size: 28px;">Cari Disini Aja !</h2>
                <h5>
                    <small class="text-secondary" style="font-size: 20px;">
                        Dapatkan informasi seputar dunia perkuliahan secara<br>terupdate dari kami melalui aplikasi
                        Kampus Indonesia
                    </small>
                </h5>
            </div>
            <div class="col-md-5 mt-3">
                <img src="" style="max-width: 90%;" alt="">
            </div>
        </div>
    </div>
</div>
@endsection